...
%  2) Provincia Picante
esPicante(Provincia):-
    habitantes(Provincia, CantidadDeHabitantes),
    CantidadDeHabitantes > 1000000,
    cantidadDePartidos(Provincia, CantidadPartidos),
    CantidadPartidos >= 2.

cantidadDePartidos(Provincia, CantidadPartidos):-
    findall(Partido, sePostula(Partido, Provincia), Partidos),
    length(Partidos, CantidadPartidos). 


%   3) leGana/3

leGana(_, OtroCandidato, Provincia):- %Gana porque el contrario no se postula
    not(compiteEnProvincia(OtroCandidato, Provincia)).

leGana(Candidato, OtroCandidato,_):-    %Gana porque son del mismo partido
    sonMismoPartido(Candidato, OtroCandidato).

leGana(Candidato, OtroCandidato, Provincia):- %Gana por intencion de votos
    compiteEnProvincia(Candidato, Provincia),
    tieneMayorIntencionDeVotos(Candidato, OtroCandidato, Provincia).
...
tieneMayorIntencionDeVotos(Candidato, OtroCandidato, Provincia) :-
    candidato(Candidato, PartidoCandidato),
    candidato(OtroCandidato, PartidoOtroCandidato),
    intencionDeVotoEn(Provincia, PartidoCandidato, PorcentajeCandidato),
    intencionDeVotoEn(Provincia, PartidoOtroCandidato, PorcentajeOtroCandidato),
    PorcentajeCandidato > PorcentajeOtroCandidato.
